  ...
  <div v-else class="main-container-one">
    <div class="main-item">
      <div class="w3-container">
        <h2 class="w3-center">Liste des Types de cheptel</h2>
      </div>
      <div v-if="typeCs">
        <div class="header-actions">
          ...
          <span class="w3-button add-up" @click="goToAddPage">
            <font-awesome-icon icon="plus" /> Ajouter un type de cheptel
          </span>
        </div>
...
const fetchTypes = () => {
  auth.axiosInstance
    ...
    .then((response) => {
      ...
      console.log("tcheptels.value:", typeCs.value);
    })
    .catch((error) => {
    ...
    .finally(() => {
      ...
      console.log("fetchTypes done");
    });
};

const goToAddPage = () => {
  console.log("Ajouter un type de cheptel");
  router.push("/TypeCheptel/add");
};
...
function onDelete(entry) {
  console.log("Supprimer:", entry.id_type_cheptel);
  deleteTypeCheptel(entry.id_type_cheptel);
}

const deleteTypeCheptel = (id) => {
  auth.axiosInstance
    .delete(`${config.API_BASE_URL}/api/typeCheptel/${id}/`)
    .then((response) => {
      fetchTypes();
      mainStore.setSuccessMessage("Type de cheptel supprimé!");
    })
    .catch((error) => {
      console.error("There was an error!", error);
    });
