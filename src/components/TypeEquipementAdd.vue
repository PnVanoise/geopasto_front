<template>
  <div>
    ...
    <TypeEquipementForm :initialForm="form" :isEdit="false" :onSubmit="submitForm" />
  </div>
</template>
...
const submitForm = async (formData) => {
  try {
    const response = await auth.axiosInstance.post(
      `${config.API_BASE_URL}/api/typeEquipement/`,
      formData
    );
    console.log("Form successfully submitted, response:", response);

    // Afficher un message de succès
    mainStore.setSuccessMessage("Type d'equipement ajouté avec succès!");
    ...
    mainStore.setErrorMessage(
      "Une erreur s'est produite lors de l'ajout du type d'équipement'." + error
    );
