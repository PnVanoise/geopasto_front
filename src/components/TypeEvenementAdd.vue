<template>
  <div>
    ...
    <TypeEvenementForm :initialForm="form" :isEdit="false" :onSubmit="submitForm" />
  </div>
</template>
...
const submitForm = async (formData) => {
  try {
    const response = await auth.axiosInstance.post(
      `${config.API_BASE_URL}/api/typeEvenement/`,
      formData
    );
    console.log("Form successfully submitted, response:", response);

    // Afficher un message de succès
    mainStore.setSuccessMessage("Type d'événement ajouté avec succès!");
    ...
    mainStore.setErrorMessage(
      "Une erreur s'est produite lors de l'ajout du type d'événement'." + error
    );
