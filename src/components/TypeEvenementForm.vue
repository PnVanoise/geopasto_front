...
onMounted(() => {
  console.log("TypeEvenementForm component mounted");
  ...
  if (!props.isEdit) {
    auth.axiosInstance
      ...
      .then((response) => {
        ...
        form.value.id_type_evenement = nextId.value; // Optionnel: lier cet ID au formulaire si besoin
      })
      .catch((error) => {
...
onBeforeUnmount(() => {
  console.log("TypeEvenementForm component before unmount");
});
</script>
