...
onMounted(() => {
  console.log("ProprietaireFoncierForm component mounted");
  ...
  if (!props.isEdit) {
    auth.axiosInstance
      ...
      .then((response) => {
        ...
        form.value.id_proprietaire = nextId.value; // Optionnel: lier cet ID au formulaire si besoin
      })
      .catch((error) => {
...
onBeforeUnmount(() => {
  console.log("ProprietaireFoncierForm component before unmount");
});
</script>
