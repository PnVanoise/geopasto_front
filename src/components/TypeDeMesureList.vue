  ...
  <div v-else class="main-container-one">
    <div class="main-item">
      <div class="w3-container">
        <h2 class="w3-center">Liste des types de mesure</h2>
      </div>
      <div v-if="typeMesures">
        <div class="header-actions">
          <form id="search" class="search-form">
            Rechercher <input name="query" v-model="searchQuery" />
          </form>
          <span class="w3-button add-up" @click="goToAddPage">
            <font-awesome-icon icon="plus" /> Ajouter un Type de mesure
          </span>
        </div>
...
const fetchTypeMesures = () => {
  auth.axiosInstance
    ...
    .then((response) => {
      ...
      console.log("typeMesures.value:", typeMesures.value);
    })
    .catch((error) => {
    ...
    .finally(() => {
      ...
      console.log("fetchTypeMesures done");
    });
};

const goToAddPage = () => {
  console.log("add typeMesure");
  router.push("/TypeMesure/add");
};
...
function onDelete(entry) {
  console.log("Supprimer:", entry.id_type_mesure);
  deleteTypeMesure(entry.id_type_mesure);
}

const deleteTypeMesure = (id) => {
  auth.axiosInstance
    .delete(`${config.API_BASE_URL}/api/typeMesure/${id}/`)
    .then((response) => {
      fetchTypeMesures();
      mainStore.setSuccessMessage("Type de mesure supprimé!");
    })
    .catch((error) => {
      console.error("There was an error!", error);
    });
