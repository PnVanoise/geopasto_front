...
onMounted(() => {
  console.log("TypeDeConventionForm component mounted");

  if (!props.isEdit) {
    auth.axiosInstance
      .get(`${config.API_BASE_URL}/api/typeConvention/getNextId/`)
      .then((response) => {
        nextId.value = response.data.next_id;
        form.value.id_type_convention = nextId.value; // Optionnel: lier cet ID au formulaire si besoin
      })
      .catch((error) => {
...
onBeforeUnmount(() => {
  console.log("TypeDeConventionForm component before unmount");
});
</script>
