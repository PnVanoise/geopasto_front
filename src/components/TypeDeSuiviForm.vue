...
onMounted(() => {
  console.log("TypeDeSuiviForm component mounted");

  if (!props.isEdit) {
    auth.axiosInstance
      .get(`${config.API_BASE_URL}/api/typeSuivi/getNextId/`)
      .then((response) => {
        nextId.value = response.data.next_id;
        form.value.id_type_suivi = nextId.value; // Optionnel: lier cet ID au formulaire si besoin
      })
      .catch((error) => {
